...
ASSUMPTIONS
{
    ASSUME(gMovesInfo[MOVE_STELLAR_STARSTORM].effect == EFFECT_TERA_STARSTORM);
}

SINGLE_BATTLE_TEST("Tera Starstorm changes from Normal-type to Stellar-type if used by Terapagos-Stellar")
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_STELLAR_STARSTORM].type == TYPE_NORMAL);
        PLAYER(SPECIES_TERAPAGOS_STELLAR);
        OPPONENT(SPECIES_MISDREAVUS);
    } WHEN {
        TURN { MOVE(player, MOVE_STELLAR_STARSTORM); }
    } SCENE {
        MESSAGE("Terapagos used Tera Starstorm!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_STELLAR_STARSTORM, player);
        HP_BAR(opponent);
...
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_STELLAR_STARSTORM].target == MOVE_TARGET_SELECTED);
        PLAYER(SPECIES_TERAPAGOS_STELLAR);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_STELLAR_STARSTORM, target:opponentLeft); }
    } SCENE {
        MESSAGE("Terapagos used Tera Starstorm!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_STELLAR_STARSTORM, playerLeft);
        HP_BAR(opponentLeft);
...
SINGLE_BATTLE_TEST("Tera Starstorm becomes a physical move if the user is Terapagos-Stellar, is Terastallized, and has a higher Attack stat", s16 damage)
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_STELLAR_STARSTORM].category == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(SPECIES_TERAPAGOS_STELLAR) { Attack(100); SpAttack(50); }
        OPPONENT(SPECIES_WOBBUFFET) { Defense(200); SpDefense(200); }
    } WHEN {
        TURN { MOVE(player, MOVE_STELLAR_STARSTORM, gimmick: tera); }
    } SCENE {
        MESSAGE("Terapagos used Tera Starstorm!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_STELLAR_STARSTORM, player);
...
SINGLE_BATTLE_TEST("Tera Starstorm remains Normal-type if used by Pokemon other than Terapagos")
{
    GIVEN {
        ...
        TURN { MOVE(player, MOVE_STELLAR_STARSTORM, gimmick: GIMMICK_TERA); }
    } SCENE {
