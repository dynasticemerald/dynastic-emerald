...
bool32 IsAffectedByFollowMe(u32 battlerAtk, u32 defSide, u32 move)
{
    ...
    if (gSideTimers[defSide].followmeTimer == 0
        ...
        || gMovesInfo[move].effect == EFFECT_BOOST_CRIT
            ...
            || (GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_STORM_DRAIN && moveType == TYPE_WATER)))
    {
        ...
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            if (side != GetBattlerSide(battler)
                ...
                && gMovesInfo[gCurrentMove].effect != EFFECT_BOOST_CRIT
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_TRUANT:
                ...
                if (!BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK && gDisableStructs[gBattlerAttacker].truantCounter))
                {
                    gBattleMoveDamage = GetNonDynamaxMaxHP(battler) / 8;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    {
    case EFFECT_FACADE:
        if (gBattleMons[battlerAtk].status1 & (STATUS1_BURN | STATUS1_PSN_ANY | STATUS1_PARALYSIS | STATUS1_FROSTBITE) || atkAbility == ABILITY_COMATOSE)
    ...
    case ABILITY_RIVALRY:
        if (AreBattlersOfSameType(battlerAtk, battlerDef))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    ...
    case ABILITY_SAND_SONG:
        if (moveType == TYPE_GROUND && gMovesInfo[move].soundMove && gBattleStruct->ateBoost[battlerAtk])
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
...
uq4_12_t CalcTypeEffectivenessMultiplier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, u32 defAbility, bool32 recordAbilities)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        else if (gBattleMons[battlerAtk].ability == ABILITY_GROUNDED_FLIGHT && moveType == TYPE_FLYING)
...
void TryRestoreHeldItems(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9 || gBattleStruct->itemLost[B_SIDE_PLAYER][i].stolen || returnNPCItems)
        {
            ...
            if (((lostItem != ITEM_NONE || returnNPCItems) && ItemId_GetPocket(lostItem) != POCKET_BERRIES ) || (ItemId_GetPocket(lostItem) == POCKET_BERRIES))
...
bool32 AreBattlersOfSameType(u32 battler1, u32 battler2)
{
    ...
    return (type1 != TYPE_MYSTERY && type2 != TYPE_MYSTERY && type1 == type2);
}
