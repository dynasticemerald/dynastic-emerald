...
static s32 (*const sBattleAiFuncTable[])(u32, u32, u32, s32) =
{
    ...
    [20] = NULL,                     // Unused
    [21] = NULL,                     // Unused
    [22] = AI_PredictSwitch,         // AI_FLAG_PREDICT_SWITCH
    [23] = NULL,                     // Unused
    [24] = NULL,                     // Unused
    [25] = NULL,                     // Unused
    [26] = NULL,                     // Unused
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_STEEL_SPIKES:
            if (gSideTimers[GetBattlerSide(battlerDef)].steelSpikesAmount > 0
              || PartnerMoveIsSameNoTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove)) //Only one mon needs to set up Stealth Rocks
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_PROTECT:
            {
                  ...
                  && move != MOVE_CRAFTY_SHIELD) //These moves have infinite usage
                {
                    if (GetBattlerSecondaryDamage(battlerAtk) >= gBattleMons[battlerAtk].hp
                      ...
                      && aiData->abilities[battlerDef] != ABILITY_SWARMING_NEIGH
...
static s32 AI_TryToFaint(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (CanIndexMoveFaintTarget(battlerAtk, battlerDef, movesetIndex, 0) && gMovesInfo[move].effect != EFFECT_EXPLOSION)
    {
        ...
        {
            if(RandomPercentage(RNG_AI_KILL_PERCENTAGE, 70))
            {
                ADJUST_SCORE(FAST_KILL_70);
            }
            else
            {
                ADJUST_SCORE(FAST_KILL_30);
            }
        }
        else if(AI_IsSlower(battlerAtk, battlerDef, move))
        {
            if(RandomPercentage(RNG_AI_KILL_PERCENTAGE, 70))
            {
                ADJUST_SCORE(SLOW_KILL_70);
            }
            else
            {
                ADJUST_SCORE(SLOW_KILL_30);
            }
        }
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_LASER_FOCUS:
        if (aiData->abilities[battlerAtk] == ABILITY_SUPER_LUCK
          ...
          || aiData->abilities[battlerAtk] == ABILITY_ARCHER
    ...
    case EFFECT_CHILLY_RECEPTION:
        ...
        else //Double Battle
        {
            ...
            if (AI_DATA->abilities[battlerDef] == ABILITY_INTIMIDATE && PartyHasMoveCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL))
                ADJUST_SCORE(7);
        }
        break;
    case EFFECT_BATON_PASS:
    ...
    case EFFECT_TAILWIND:
        if (GetBattlerSideSpeedAverage(battlerAtk) < GetBattlerSideSpeedAverage(battlerDef))
            ADJUST_SCORE(GOOD_EFFECT);
        else if (AI_DATA->abilities[battlerAtk] == ABILITY_PRANKSTER 
        && gDisableStructs[battlerAtk].isFirstTurn)
            ADJUST_SCORE(PRANKSER_EFFECT);
        break;
    case EFFECT_LUCKY_CHANT:
...
static s32 AI_PredictSwitch(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if ((ability == ABILITY_STAKEOUT || ability == ABILITY_ANALYTIC) && IS_MOVE_STATUS(battlerAtk))
        ADJUST_SCORE(-WEAK_EFFECT);
